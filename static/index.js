...
d3.json("/data.json").then(result => {
  ...
  const button = d3.select("#books")
    .append("button")
    .text("Show all books")
    .on("click", function (){ renderBooks()} )
...
function renderLanguage() {
  ...
  function zoomTo(v) {
    ...
    const showLanguage = d3.select(".language")
      .append("button")
      .text("Show books per language")
      .on("click", function (){ renderLanguage()} )
}

  // function onChange(){
  //   console.log("onChange");
  //   const button = d3.select(".language")
  //     .append("button")
  //     .text("Show all books")
  //     .on("click", function (){ renderBooks()} )
  // }
  ...
  function renderBooks() {
    ...
    // setup circle pack
    // setup with Folkert-Jan vd Pol, finished setup + fixed hierarchy with Titus Wormer
    const root = d3.hierarchy(dataBooks)
      .sum(d => d.totalPages)
      .sort((a, b) => b.value - a.value)
